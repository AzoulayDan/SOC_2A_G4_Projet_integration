...
class ConcetCompte: Compte {
    static func create_account(user_name:String?, user_lastname:String?, user_email:String?, user_password:String?, user_credit:String?, user_account_id:String?, user_favorite_sports:[String]?) -> Compte {
        ...
        let newAccount = NSEntityDescription.insertNewObject(forEntityName: "Compte", into: moc) as! Compte
        
        newAccount.compte_username = user_name
        newAccount.compte_user_lastname = user_lastname
        newAccount.compte_mail = user_email
        newAccount.compte_password = user_password
        newAccount.compte_montant = user_credit
        newAccount.compte_id_in_Mongo_BDD = user_account_id
        
        if (user_favorite_sports == nil) {
            newAccount.compte_sports = nil
        }
        else {
            newAccount.compte_sports = NSKeyedArchiver.archivedData(withRootObject: user_favorite_sports!) as NSData
        }
